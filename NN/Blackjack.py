...
def hand( montecarlo, level, debug ):
	...
	if summ == 21 and hand_value( dealers_hand ) == 21:
		if debug:
			print( "21 each." )
			print( "TIE" )
		return data, tags
	elif summ == 21 and hand_value( dealers_hand ) != 21:
		if debug:
			print( "BlackJack!" )
			print( "PLAYER WINS" )
		return data, tags
...
def test_model( model_name, num_of_tests, fresh_deck, level, debug ):
	...
	for i in range( num_of_tests ):
		...
		for j in range(8):
			...
			prediction = model.predict( np.array( data ) , verbose = 0) #AI decision to hit or stand
...
def gen_data_set( num_of_games, name, level, shuffle = True):
